   ...
   for (int subLine = 0; subLine < CELL/2; subLine++)
   {
      ...
      for (int iPair = 0; iPair < 4; iPair++)
      {
         ...
         for (int subColumn = 0; subColumn < CELL; subColumn++)
         {
            ...
            {
               cout << char(game.checkPieceAtPosition(iLine, iPair*2) != 0x20 ? game.checkPieceAtPosition(iLine, iPair*2) : iColor1);
            }
            else
            {
               cout << char(iColor1);
         ...
         for (int subColumn = 0; subColumn < CELL; subColumn++)
         {
            ...
            if ( subLine == 1 && subColumn == 3)
            {
              cout << char(game.checkPieceAtPosition(iLine,iPair*2+1) != 0x20 ? game.checkPieceAtPosition(iLine,iPair*2+1) : iColor2);
            }
            else
            {
               cout << char(iColor2);
